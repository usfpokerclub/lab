...
config_object_str = "server.config.DevConfig"
app.config.from_object(config_object_str)
db = SQLAlchemy(app)
migrate = Migrate(app, db)
sslify = SSLify(app)
socketio = SocketIO(
    app, message_queue=app.config["MESSAGE_QUEUE_URL"], cors_allowed_origins="*"
)
...
def make_celery(flask_app):
    celery = Celery(
        flask_app.import_name, broker=flask_app.config["CELERY_BROKER_URL"]
    )
